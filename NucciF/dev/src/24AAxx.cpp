...
/*!****************************************************************************
*/
Eep24AA::Eep24AA(i2cRead_t _read, i2cWrite_t _write):
	m_read(_read), m_write(_write){}

/*!****************************************************************************
*/
void Eep24AA::setI2c(i2cRead_t _read, i2cWrite_t _write){
	m_read = _read;
	m_write = _write;
}
...
bool Eep24AA::write(uint16_t dst, const void *src, uint16_t len){
	...
	while(len > 0){
		...
		if(len <= canWrite){
			...
			if(!m_write(eepAdr.all, eep_tempBff, len + 1, true, i2ctimeout)){
				return false;
			}
			...
			if(!m_write(eepAdr.all, eep_tempBff, canWrite + 1, true, i2ctimeout)){
				return false;
			}
			vTaskDelay(eepdelayms);
			len -= canWrite;
...
bool Eep24AA::read(void *dst, uint16_t src, uint16_t len){
	...
	if(!m_write(eepAdr.all, &adrInBlock, 1, true, i2ctimeout)){
		return false;
	}

	eepAdr.bit.rw = eepRead;

	if(!m_read(eepAdr.all, (uint8_t*)dst, len, i2ctimeout)){
		return false;
	}
