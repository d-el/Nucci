 ...
 *
 */
void disp_keepBackgroundColor(bool useBackground){
	uiUseBackground = useBackground;
}
/*!****************************************************************************
 *
 */
 ...
 *
 */
disp_color_type disp_getPixel(uint16_t x, uint16_t y){
	return st7735_getPixel(x, y);
}
	...
	for(height = 0; height < c_height; height++){
		...
		for(width = 0; width < bytesPerLine; width++){
			...
			for(int j = 0; j < count; j++){
				if((*pdata & mask) != 0){
					disp_color_type back = uiUseBackground ? disp_getPixel(x + (width * 8) + j, y + height) :
															uiBackgroundColor;
					disp_setPixel(x + (width * 8) + j, y + height, back);
				}else{
