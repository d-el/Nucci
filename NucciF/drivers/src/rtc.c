...
bool rtc_backupWrite(const uint32_t* src, uint8_t dstoffset, size_t len){
	...
	const uint32_t* pR = (const uint32_t*)src;
	uint32_t* pW = (uint32_t*)&RTC->BKP0R + dstoffset;
	uint32_t* pWend = pW + len;
	while(pW < pWend){
		*pW++ = *pR++;
	}
...
bool rtc_backupRead(uint8_t srcoffset, uint8_t* dst, size_t len){
	const uint8_t* pR = (const uint8_t*)&RTC->BKP0R + srcoffset;
	uint8_t* pW = (uint8_t*)dst;
	uint8_t* pWend = pW + len;
	while(pW < pWend){
		*pW++ = *pR++;
	}
	return true;
